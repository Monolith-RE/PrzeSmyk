...
# ==============================================================================
# 3. SILNIK GIS Z ELASTYCZNYM PROMIENIEM I BUFOREM 2.5M
# ==============================================================================
...
def geokoduj_wpis_startowy(tekst_wpisu):
    ...
    try:
        ...
        r = requests.get(url, headers={'User-Agent': 'PrzeSmykApp/10.0'}, timeout=3)
...
def pobierz_adres_i_filtr_zabudowy(lat, lon, nr_dzialki_ewidencja):
    try:
        ...
        r = requests.get(url, headers={'User-Agent': 'PrzeSmykApp/10.0'}, timeout=2)
        if r.status_code == 200:
            data = r.json()
...
# ==============================================================================
st.sidebar.title("🚙⚡🔥 PrzeSmyk v2.6")
...
if przelicz_button:
    ...
    with st.spinner("⏳ Trwa skanowanie obszaru przesyłowego w głąb powiatów..."):
        ...
        for promien in PROMIENIE:
            ...
            sieci_w_promieniu = sieci[sieci.geometry.intersects(bufor_obszaru)].copy()
            ...
            for idx, linia in sieci_w_promieniu.iterrows():
                ...
                for d in range(0, int(dlugosc), 120):
                    ...
                    if dzialka:
                        ...
                        # ZMNIEJSZONY BUFOR DO 2.5 METRA DLA DOMÓW JEDNORODZINNYCH
        ...
        if not df.empty:
            ...
            st.success(f"✅ Wygenerowano marszrutę! Znaleziono {len(df)} idealnych działek jednorodzinnych/firm.")
        else:
            st.warning("Nie znaleziono działek spełniających kryteria w badanym obszarze.")
